    ...
    def evaluar(self, x):
        ...
        """
            if self.posicion(x) == self.posicion(self.objetivos[i]):
                eval.append(self.F[i] * l2(x, self.objetivos[i]))
            else:
                if self.posicion(x) == 1:
                    D1 = l2(x, self.p1n) + self.diam + l2(self.p1s, self.objetivos[i])
                    D2 = l2(x, self.p2n) + self.diam + l2(self.p2s, self.objetivos[i])
                    eval.append(self.F[i] * min(D2, D1))
                elif self.posicion(x) == -1:
                    D1 = l2(x, self.p1s) + self.diam + l2(self.p1n, self.objetivos[i])
                    D2 = l2(x, self.p2s) + self.diam + l2(self.p2n, self.objetivos[i])
                    eval.append(self.F[i] * min(D2, D1))
        """
